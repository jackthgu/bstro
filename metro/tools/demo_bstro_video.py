...
def export_contact_obj(pred_contact, smpl, fname, device='cuda'):
    """
    간단한 예시: 주어진 pred_contact (6890, )를 기준으로 마디별 색상 지정 후 obj 저장
    """
    import trimesh
    # T-pose 템플릿
    ref_vert = smpl(torch.zeros((1,72)).to(device), torch.zeros((1,10)).to(device)).squeeze(0)
    faces = smpl.faces.detach().cpu().numpy()
    contact = pred_contact.detach().cpu()
    mesh = trimesh.Trimesh(vertices=ref_vert.detach().cpu().numpy(), faces=faces, process=False)
    
    # contact >= 0.5인 버텍스에 색상 지정 (빨간색)
    hit_id = (contact >= 0.5).nonzero(as_tuple=True)[0]
    mesh.visual.vertex_colors = (191, 191, 191, 255)
...
def run_video_inference(args, BSTRO_model, smpl, mesh_sampler):
    """
    Video를 입력받아 frame별 추론 → .obj 파일, frame별 이미지 파일로 저장
    그리고 처리 속도(average FPS)를 출력
    """
    smpl.eval()
    BSTRO_model.eval()
    ...
    if not cap.isOpened():
        print(f"Failed to open video: {args.input_video}")
        return
    ...
    while True:
        ...
        # 전처리
        img = rgb_processing(frame_rgb, center, sc * scale, rot, pn, img_res=224)
        img_tensor = torch.from_numpy(img).float()
        img_normalized = normalize_img(img_tensor)
        img_normalized = img_normalized.cuda(args.device).unsqueeze(0)

        with torch.no_grad():
            # 추론
            if BSTRO_model.config.output_attentions:
                pred_sub2, pred_sub, pred_contact, hidden_states, att = \
                    BSTRO_model(img_normalized, smpl, mesh_sampler)
            else:
                pred_sub2, pred_sub, pred_contact = \
                    BSTRO_model(img_normalized, smpl, mesh_sampler)

            # .obj 저장
            obj_fname = os.path.join(foldername, f"contact_vis_{frame_idx:04d}.obj")
            export_contact_obj(pred_contact[0], smpl, obj_fname, device=args.device)
            print(pred_contact[0])



        # 프레임 이미지도 저장 (원본 BGR)
        out_img_path = os.path.join(foldername, f"frame_{frame_idx:04d}.jpg")
    ...
    print(f"Processed {total_frames} frames in {total_time:.2f} sec.")
...
def parse_args():
    ...
    parser.add_argument('-a', '--arch', default='hrnet-w32',
                        help='CNN backbone architecture: hrnet-w18, hrnet-w32, hrnet-w64, hrnet, resnet50')
    ...
    parser.add_argument("--num_attention_heads", default=2, type=int, required=False, 
                        help="Update model config if given (reduced for speed). "
                             "Note that hidden_size / num_attention_heads should be integer.")
...
def main(args):
    ...
    run_video_inference(args, _bstro_network, smpl, mesh_sampler)
